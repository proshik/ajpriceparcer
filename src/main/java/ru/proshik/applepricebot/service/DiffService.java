...
public class DiffService {
    ...
    public Map<ShopType, Map<ProductType, List<PriceProductDifferent>>> buildPriceDifferent(Map<ShopType, List<ru.proshik.applepricebot.repository.model.Product>> products, LocalDateTime date) {
        Map<ShopType, Map<ProductType, List<PriceProductDifferent>>> result = new HashMap<>();

        Map<ShopType, List<ru.proshik.applepricebot.repository.model.Product>> prevGroupByShop = productRepository.findByFetchDate(date).stream()
                .collect(Collectors.groupingBy(ru.proshik.applepricebot.repository.model.Product::getShopType));

        for (Map.Entry<ShopType, List<ru.proshik.applepricebot.repository.model.Product>> entry : products.entrySet()) {

            Map<ProductType, List<ru.proshik.applepricebot.repository.model.Product>> prevProductGropByProductType = prevGroupByShop.get(entry.getKey()).stream()
                    .collect(Collectors.groupingBy(ru.proshik.applepricebot.repository.model.Product::getProductType));

            Map<ProductType, List<ru.proshik.applepricebot.repository.model.Product>> newProductsGroupByProductType = entry.getValue().stream()
                    .collect(Collectors.groupingBy(ru.proshik.applepricebot.repository.model.Product::getProductType));

            Map<ProductType, List<PriceProductDifferent>> productTypeResult = new HashMap<>();
            for (Map.Entry<ProductType, List<ru.proshik.applepricebot.repository.model.Product>> currProductEntry : newProductsGroupByProductType.entrySet()) {

                Map<String, List<ru.proshik.applepricebot.repository.model.Product>> previousGroupByTitle = prevProductGropByProductType.get(currProductEntry.getKey()).stream()
                        .collect(Collectors.groupingBy(ru.proshik.applepricebot.repository.model.Product::getTitle));

                List<PriceProductDifferent> priceProdDiffResult = new ArrayList<>();
                for (ru.proshik.applepricebot.repository.model.Product p : currProductEntry.getValue()) {
                    List<ru.proshik.applepricebot.repository.model.Product> findProductsByTitle = previousGroupByTitle.get(p.getTitle());
                    ...
                    if (findProductsByTitle.size() > 1) {
                        ru.proshik.applepricebot.repository.model.Product prevProduct = findProductsByTitle.get(0);
